...
def split_data(wav_in, wav_out, mid_in, mid_out):
    ...
    total_files = len(glob.glob1(wav_in,"*.wav"))

    # Use a random permutation to randomly select files fro training and testing
    rand_perm = np.random.permutation(total_files + 1)
    
    num_train_recordings = total_files*0.8
    num_test_recordings = total_files*0.2

    # For each file in the audio folder, find its associated midi file and put them both in
    # either training or testing folders
    for i, wav_file in enumerate(glob.glob1(wav_in,"*.wav")):
        ...
        if rand_perm[i] < num_train_recordings:
            dest = 'train'
